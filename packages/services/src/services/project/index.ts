...
import type {
  ...
  ProjectDetailsDTO,
  ProjectSummaryDTO
} from '@services/services/project/types'
import {
  DEFAULT_MEMBER_ROLE,
  assertProjectRole,
  isSystemAdmin,
  requireSystemRole
} from '@services/services/project/roles'
import { mapProjectDetails, mapProjectSummary } from '@services/services/project/helpers'
  ...
  async createProject(actor: ProjectActor, payload: unknown): Promise<ProjectDetailsDTO> {
    ...
    try {
      ...
      if (error instanceof UniqueConstraintError) {
        throw new AppError('ERR_CONFLICT', 'Esiste giÃ  un progetto con la stessa key', {
          cause: error
        })
      }
      throw wrapError(error)
...
export type { ProjectActor } from '@services/services/project/types'
